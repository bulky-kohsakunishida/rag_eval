...
def create_llm_chain(request_timeout=60, min_request_interval=DEFAULT_REQUEST_INTERVAL):
    """
    LLMチェーンを作成する
...
def answer_question_with_retry(llm, prompt, question, max_retries=5):
    """質問に回答する（リトライロジック付き）"""
    retries = 0
    ...
    while retries <= max_retries:
        try:
            ...
            return answer, []  # 空のソースドキュメントリストを返す
...
def process_qa_csv(llm, prompt, csv_path, output_path, max_retries=5):
    ...
    for i, row in df.iterrows():
        ...
        try:
            answer, _ = answer_question_with_retry(llm, prompt, question, max_retries=max_retries)
...
def resume_from_temp(llm, prompt, csv_path, output_path, temp_path, max_retries=5):
    """途中結果から処理を再開する"""
    # 途中結果を読み込む
...
def main():
    parser = argparse.ArgumentParser(description="LLMアプリケーション")
    parser.add_argument("--qa_csv", default="qa.csv", help="質問回答のCSVファイル")
    parser.add_argument("--output", default="vanilla_llm_results.csv", help="結果の出力先")
    ...
    if args.interactive:
        # 対話モード
        print("LLMアプリケーションを起動しました。終了するには 'exit' と入力してください。")
        while True:
            question = input("\n質問を入力してください: ")
            ...
            try:
                answer, _ = answer_question_with_retry(llm, prompt, question, max_retries=args.max_retries)
                print("\n回答:")
                print(answer)
        ...
        if args.resume:
            ...
            if os.path.exists(temp_path):
                ...
                resume_from_temp(llm, prompt, args.qa_csv, args.output, temp_path, max_retries=args.max_retries)
            else:
                print(f"途中結果ファイル {temp_path} が見つかりません。最初から処理を開始します")
                process_qa_csv(llm, prompt, args.qa_csv, args.output, max_retries=args.max_retries)
        else:
            # 通常のCSV処理
            process_qa_csv(llm, prompt, args.qa_csv, args.output, max_retries=args.max_retries)
